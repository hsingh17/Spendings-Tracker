...
public class SpendingServiceImpl implements SpendingService {
    ...
    public Page<SpendingsListProjection> getSpendings(
            long userId, Date startDate, Date endDate, int page, int limit, GroupBy groupBy, SpendingType type) {
    ...
    public void updateSpending(Set<Spending> spendings, Date spendingDate, long userId) {
        ...
        if (spendingsToKeep.isEmpty()) { // User decided to delete all the spendings, therefore, this is effectively a delete operation
    ...
    private Set<Spending> filterSpendings(Set<Spending> spendings) {
        Map<String, List<Spending>> mappedSpendings = spendings
                .stream()
                .filter(spending -> !spending.isDelete())
                .collect(Collectors.groupingBy(Spending::getCategory)); // Group by category in case there are multiple spendings in the same category
        ...
        for (List<Spending> mergedSpendings : mappedSpendings.values()) {
            Optional<Spending> mergedSpending = mergedSpendings
                    .stream()
                    .reduce(Spending::merge);
