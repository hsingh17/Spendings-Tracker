...
const SortIcon: FC<SortIconProps> = ({ sortOrder }) => {
  ...
  return (
    <div className="flex flex-col">
      <SortUp className={className} stroke="white" fill={sortOrder === SortOrder.ASC ? "white" : "gray"} />
      <SortDown className={`${className} -mt-[22.5px]`} stroke="white" fill={sortOrder === SortOrder.DESC ? "white" : "gray"} />
    </div>
  );
};
